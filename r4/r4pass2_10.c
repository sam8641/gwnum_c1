...
void r4_pass2_10_levels_complex(struct gwasm_data *__restrict g);
void xpass2_r4_10_levels(struct gwasm_data *__restrict g) {
	g->thread_work_routine = r4_pass2_10_levels_complex;
	pass2_wake_up_threads(g);
	if(g->NEGACYCLIC_FFT == 0) {
//static inline void r4_pass2_10_levels_real(struct gwasm_data *__restrict g) {
	...
	if likely(!(g->ffttype & 4)) {
		...
		for(unsigned int loopA = 2; loopA; loopA--) {
			for(unsigned int loopB = 128; loopB; loopB--) {
				r4_fh2cl_eight_reals_four_complex_djbfft(rsi, 64, 2*dist128, rdi, rdx);
		...
		rdx += -256*XMM_HSCD2;	/* Restore sin/cos pointer */
		...
		rsi = daddr; //rsi += -2*2*dist128;	/* Next source pointer */
		...
		/* BUG in comment: (4×2−1)×16 = 112 macros */
		...
		rsi = daddr; //rsi += -4*dist128;		/* Next source pointer */
		...
		r4_x4cl_four_complex_djbfft_preload;
		for(unsigned int loopA = 4*8-1; loopA; ) {
			do{
		...
		if likely(g->ffttype & 2) {
			if likely(!(g->ffttype & 1)) { //xpass2_10_levels_real_2:;
				...
				for(unsigned int loopA = 4*32-1; loopA; ) {
					do{
						r4_x4cl_four_complex_with_square(rsi, 4*64, 64, 2*64);
					}while(--loopA & 31);
					rsi += -32*4*64+dist128;	/* Next source pointer */
				}
				//rsi += -4*dist128;		/* Next source pointer */
				pfing(rcx += 128);		/* Skip pad bytes */
			}else{ //xpass2_10_levels_real_3:;
	...
	loops_init_prefetch(128, 64, 2, rcx);
	xtouch(xptr(rcx+4096-64));		/* Load the prefetch TLB */
	for(unsigned int loopA = 2; loopA; loopA--) {
	...
	for(unsigned int loopA = 4*16-1; loopA; ) {
		do{
	...
	for(unsigned int loopA = 32; loopA; loopA--) {
		r4_h4cl_eight_reals_four_complex_djbunfft(rsi, 64, 32*64, 64*64, rdi, 32*XMM_SCD2, rdx, 128*XMM_HSCD2);
		rdi += XMM_SCD2;		/* Next sin/cos data */
		rdx += 4*XMM_HSCD2;	/* Next sin/cos data */
	}
	...
	for(unsigned int loopA = 128; loopA; loopA--) {
		r4_h4cl_eight_reals_four_complex_djbunfft(rsi, 64, dist128, 2*dist128, rdi, 128*XMM_SCD2, rdx, 128*XMM_HSCD2);
		rdi += XMM_SCD2;		/* Next sin/cos data */
		rdx += XMM_HSCD2;		/* Next sin/cos data */
	}
...
do{
	...
	if likely(!(g->ffttype & 4)) {
		...
		for(unsigned int loopA = 2; loopA; loopA--) {
			for(unsigned int loopB = 4; loopB; loopB--) {
				...
				pfing(rcx += -8*64+4096);		/* Prefetch from next 4KB page */
				...
				pfing(rcx += -8*64+4096+128);	/* Prefetch from next 4KB page */
				...
				pfing(rcx += -(8192+128));	/* Back to prefetching from 1st 4KB page */
		...
		rsi = daddr; //rsi += -2*2*dist128;	/* Restore source pointer */
		pfing(rcx += 8192+128);		/* Move prefetch pointer along */
		end_timer(6);
		...
		if likely(g->ffttype & 2) {
			if likely(!(g->ffttype & 1)) { //xpass2_10_levels_complex_2:;
				start_timer(9);
				loops_init_prefetch(128, 64, 2, rcx);
				xtouch(xptr(rcx+4096-64));		/* Preload the TLB */
				r4_x4cl_four_complex_with_square_preload;
				...
				pfing(rcx += 128);		/* Skip pad bytes */
				end_timer(9);
			}else{ //xpass2_10_levels_complex_3:;
				start_timer(9);
				rbp = g->DIST_TO_MULSRCARG;
				loops_init_prefetch(128, 64, 2, rcx);
				xtouch(xptr(rcx+4096-64));		/* Preload the TLB */
		...
		}else{ //xpass2_10_levels_complex_1:;
			start_timer(9);
	...
	}else{//xpass2_10_levels_complex_4:;
		start_timer(9);
		pfing(rcx = (uintptr_t)g->data_prefetch);/* Prefetch pointer */
		xtouch(xptr(rcx));			/* Load first prefetch TLB */
		rbp = g->DIST_TO_MULSRCARG;
	...
	xtouch(xptr(rcx+4096-64));		/* Load 3 prefetch TLBs */
	xtouch(xptr(rcx+2*4096-64));
	xtouch(xptr(rcx+3*4096-64));
	...
	r4_x4cl_four_complex_djbunfft_preload;
	for(unsigned int loopA = 128; loopA; loopA--) {
		r4_x4cl_four_complex_djbunfft(rsi, 64, dist128, 2*dist128, rdi, 128*XMM_SCD2);
