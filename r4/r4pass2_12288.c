...
//void r4_pass2_12288_levels_real(struct gwasm_data *__restrict g)
	...
	if(g->ffttype != 4) { //xpass2_12288_levels_real_4;
		...
		rsi += -3*16*dist128;	/* Restore source pointer */
		...
		rsi += -6*8*dist128;	/* Restore source pointer */
		...
		rsi += -24*2*dist128;	/* Restore source pointer */
		...
		rsi += -48*dist128;	/* Next source pointer */
		...
		if(g->ffttype <= 2) {
			if(g->ffttype == 2) { // xpass2_12288_levels_real_2:;
	...
	rsi += -12*4*dist128;	/* Next source pointer */
...
do{
	...
	rbx = g->DIST_TO_FFTSRCARG;
	if(g->ffttype != 4) { //xpass2_12288_levels_complex_4;
		...
		if(g->ffttype <= 2) {
			if(g->ffttype == 2) { // xpass2_12288_levels_complex_2:;
				start_timer(9);
				loops_init(1536);
				r4_x4cl_four_complex_with_square_preload;
	...
	rsi += -12*4*dist128;	/* Restore source pointer */
	end_timer(13);
