...
void xpass2_r4_1536_levels(struct gwasm_data *__restrict g) {
	g->thread_work_routine = r4_pass2_1536_levels_complex;
	pass2_wake_up_threads(g);
	if(g->NEGACYCLIC_FFT == 0) {

//static inline void r4_pass2_1536_levels_real(struct gwasm_data *__restrict g) {
	...
	if likely(!(g->ffttype & 4)) {
		...
		for(unsigned int loopA = 2; loopA; loopA--) {
			for(unsigned int loopB = 128; loopB; loopB--) {
				r3_fh3cl_six_reals_three_complex_djbfft(rsi, 64, 2*dist128, rdi, 256*XMM_SCD1, rdx, 256*XMM_HSCD1);
		...
		rdx += 256*XMM_HSCD1;	/* Next sin/cos ptr */
		...
		rsi = daddr; //rsi += -3*2*dist128;	/* Next source pointer */
		...
		rsi = daddr; //rsi += -6*dist128;		/* Next source pointer */
		...
		r4_x4cl_four_complex_djbfft_preload;
		for(unsigned int loopA = 6*4-1; loopA; ) {
			do{
		...
		if likely(g->ffttype & 2) {
			if likely(!(g->ffttype & 1)) { //xpass2_1536_levels_real_2:;
				...
				xtouch(xptr(rcx+4096-64));		/* Preload the TLBs */
				xtouch(xptr(rcx+2*4096-64));		/* Preload the TLBs */
				for(unsigned int loopA = 6*16-1; loopA; ) {
					do{
						r8_x8cl_eight_complex_with_square(rsi, 8*64, 64, 2*64, 4*64);
					}while(--loopA & 15);
					rsi += -16*8*64+dist128;	/* Next source pointer */
				}
				//rsi += -6*dist128;		/* Next source pointer */
			}else{ //xpass2_1536_levels_real_3:;
		...
		}else{//xpass2_1536_levels_real_1:;
			...
			goto	xpass2_1536_real_done;
		}
	}else{ //xpass2_1536_levels_real_4:;
		rbp = g->DIST_TO_MULSRCARG;
		pfing(rcx = (uintptr_t)g->data_prefetch); /* Load prefetch pointer */
		xtouch(xptr(rcx));			/* Preload first TLBs */
	...
	for(unsigned int loopA = 6*8-1; loopA; ) {
		do{
	...
	r4_x4cl_four_complex_djbunfft_preload;
	for(unsigned int loopA = 6*2-1; loopA; ) {
		do{
	...
	rdi = (uintptr_t)g->xsincos_complex;	/* Load sin/cos pointers */
	rdx = (uintptr_t)g->sincos3;
	pfing(rcx += -21*64);		/* Back up prefetch pointer by 21 cache lines */
	loops_init_prefetch(256, 64, 2, rcx);
	xtouch(xptr(rcx+4096-64));
	xtouch(xptr(rcx+2*4096-64));
	for(unsigned int loopA = 2; loopA; loopA--) {
		for(unsigned int loopB = 128; loopB; loopB--) {
			r3_h3cl_six_reals_three_complex_djbunfft(rsi, 64, 2*dist128, rdi, 256*XMM_SCD1, rdx, 256*XMM_HSCD1);
			rdi += XMM_SCD1;		/* Next sin/cos data */
			rdx += XMM_HSCD1;		/* Next sin/cos data */
		}
		rsi += -128*64+dist128;	/* Next source pointer */
...
//}
	...
	r4_pass2_1536_levels_complex(g);
}
...
do{
	...
	if likely(!(g->ffttype & 4)) {
		...
		for(unsigned int loopA = 2; loopA; loopA--) {
			for(unsigned int loopB = 128; loopB; loopB--) {
				...
				xxprefetch(u64ptr(rdi+257*XMM_SCD1));	/* Prefetch for next pass */
		...
		for(unsigned int loopA = 3; loopA; loopA--) {
			xtouch(xptr(rcx+4096-64));		/* Load 2 prefetch TLBs */
			xtouch(xptr(rcx+2*4096-64));
		...
		rsi = daddr; //rsi += -3*2*dist128;	/* Restore source pointer */
		end_timer(6);
		...
		if likely(g->ffttype & 2) {
			if likely(!(g->ffttype & 1)) { //xpass2_1536_levels_complex_2:;
				start_timer(9);
				...
				//rsi += -6*dist128;		/* Restore source pointer */
				end_timer(9);
			}else{ //xpass2_1536_levels_complex_3:;
				start_timer(9);
				rbp = g->DIST_TO_MULSRCARG;
				loops_init_prefetch(96, 64, 1, rcx);
				r8_x8cl_eight_complex_with_mult_preload;
		...
		}else{ //xpass2_1536_levels_complex_1:;
			start_timer(9);
			...
			end_timer(9);
			goto	xpass2_1536_levels_complex_done;
		}
	}else{ //xpass2_1536_levels_complex_4:;
		start_timer(9);
		pfing(rcx = (uintptr_t)g->data_prefetch);/* Prefetch pointer */
	...
	pfing(rcx += -22*64);		/* Back up prefetch pointer by 22 cache lines */
	...
	for(unsigned int loopA = 2; loopA; loopA--) {
		for(unsigned int loopB = 128; loopB; loopB--) {
			r3_x3cl_three_complex_djbunfft(rsi, 64, 2*dist128, rdi, 256*XMM_SCD1);
