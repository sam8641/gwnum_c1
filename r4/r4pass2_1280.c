...
void r4_pass2_1280_levels_complex(struct gwasm_data *__restrict g);
void xpass2_r4_1280_levels(struct gwasm_data *__restrict g) {
	g->thread_work_routine = r4_pass2_1280_levels_complex;
	pass2_wake_up_threads(g);
	if(g->NEGACYCLIC_FFT == 0) {

//static inline void r4_pass2_1280_levels_real(struct gwasm_data *__restrict g) {
	...
	if likely(!(g->ffttype & 4)) {
		...
		for(unsigned int loopA = 128; loopA; loopA--) {
			r5_fh5cl_ten_reals_five_complex_djbfft(rsi, 64, dist128, rdi, 128*XMM_SCD2, rdx, 128*XMM_HSCD2);
		...
		xtouch(xptr(rcx));			/* Load 5 prefetch TLBs */
		...
		rsi = daddr; //rsi += -5*dist128;		/* Next source pointer */
		...
		for(unsigned int loopA = 5*2-1; loopA; ) {
			do{
		...
		r4_x4cl_four_complex_djbfft_preload;
		for(unsigned int loopA = 5*8-1; loopA; ) {
			do{
		...
		if likely(g->ffttype & 2) {
			if likely(!(g->ffttype & 1)) { //xpass2_1280_levels_real_2:;
				...
				for(unsigned int loopA = 5*32-1; loopA; ) {
					do{
						r4_x4cl_four_complex_with_square(rsi, 4*64, 64, 2*64);
					}while(--loopA & 31);
					rsi += -32*4*64+dist128;	/* Next source pointer */
				}
				//rsi += -5*dist128;		/* Next source pointer */
			}else{ //xpass2_1280_levels_real_3:;
	...
	for(unsigned int loopA = 5*16-1; loopA; ) {
		do{
	...
	r4_x4cl_four_complex_djbunfft_preload;
	for(unsigned int loopA = 5*4-1; loopA; ) {
		do{
	...
	for(unsigned int loopA = 128; loopA; loopA--) {
		r5_h5cl_ten_reals_five_complex_djbunfft(rsi, 64, dist128, rdi, 128*XMM_SCD2, rdx, 128*XMM_HSCD2);
		rdi += XMM_SCD2;		/* Next sin/cos data */
		rdx += XMM_HSCD2;		/* Next sin/cos data */
	}
...
//}
	...
	r4_pass2_1280_levels_complex(g);
}
...
do{
	...
	if likely(!(g->ffttype & 4)) {
		...
		for(unsigned int loopA = 128; loopA; loopA--) {
			...
			xxprefetch(u64ptr(rdi+129*XMM_SCD2));	/* Prefetch for next pass */
		...
		if likely(g->ffttype & 2) {
			if likely(!(g->ffttype & 1)) { //xpass2_1280_levels_complex_2:;
				start_timer(9);
				loops_init_prefetch(160, 64, 4, rcx);
				r4_x4cl_four_complex_with_square_preload;
				for(unsigned int loopA = 5; loopA; loopA--) {
				...
				end_timer(9);
			}else{ //xpass2_1280_levels_complex_3:;
				start_timer(9);
				rbp = g->DIST_TO_MULSRCARG;
				loops_init_prefetch(160, 64, 4, rcx);
		...
		}else{ //xpass2_1280_levels_complex_1:;
			start_timer(9);
			...
			end_timer(9);
			goto	xpass2_1280_levels_complex_done;
		}
	}else{ //xpass2_1280_levels_complex_4:;
		start_timer(9);
		pfing(rcx = (uintptr_t)g->data_prefetch); /* Prefetch pointer */
		xtouch(xptr(rcx));			/* Load first 5 prefetch TLBs */
		rbp = g->DIST_TO_MULSRCARG;
	...
	xtouch(xptr(rcx+4096-64));		/* Load last 5 prefetch TLBs */
	xtouch(xptr(rcx+2*4096-64));
	xtouch(xptr(rcx+3*4096-64));
	xtouch(xptr(rcx+4*4096-64));
	xtouch(xptr(rcx+5*4096-64));
	...
	for(unsigned int loopA = 128; loopA; loopA--) {
		r5_x5cl_five_complex_djbunfft(rsi, 64, dist128, rdi, 128*XMM_SCD2);
