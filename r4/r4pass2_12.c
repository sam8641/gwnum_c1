...
//void r4_pass2_12_levels_real(struct gwasm_data *__restrict g)
	...
	if(g->ffttype != 4) { //xpass2_12_levels_real_4;
		...
		rsi += -2*dist128-8*dist128; /* Restore source pointer */
		...
		rsi += -8*2*dist128;	/* Next source pointer */
	...
	rsi += -4*4*dist128;	/* Next source pointer */
...
do{
	...
	if(g->ffttype != 4) { //xpass2_12_levels_complex_4;
		...
		rsi += -2*4*2*dist128;	/* Restore source pointer */
		end_timer(6);
	...
	rsi += -2*2*4*dist128;	/* Restore source pointer */
	end_timer(13);
