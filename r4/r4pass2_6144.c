...
//void r4_pass2_6144_levels_real(struct gwasm_data *__restrict g)
	...
	if(g->ffttype != 4) { //xpass2_6144_levels_real_4;
		...
		rsi += -6*4*dist128;	/* Next source pointer */
		...
		if(g->ffttype <= 2) {
			if(g->ffttype == 2) { // xpass2_6144_levels_real_2:;
	...
	rsi += -12*2*dist128;	/* Next source pointer */
	...
	rsi += -3*8*dist128;	/* Next source pointer */
...
do{
	...
	rbx = g->DIST_TO_FFTSRCARG;
	if(g->ffttype != 4) { //xpass2_6144_levels_complex_4;
		...
		rsi += -6*4*dist128;	/* Restore source pointer */
		end_timer(6);
		...
		if(g->ffttype <= 2) {
			if(g->ffttype == 2) { // xpass2_6144_levels_complex_2:;
				start_timer(9);
	...
	rsi += -12*2*dist128;	/* Restore source pointer */
	end_timer(10);
