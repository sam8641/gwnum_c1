...
void r4_pass2_5120_levels_complex(struct gwasm_data *__restrict g);
void xpass2_r4_5120_levels(struct gwasm_data *__restrict g) {
	g->thread_work_routine = r4_pass2_5120_levels_complex;
	pass2_wake_up_threads(g);
	if(g->NEGACYCLIC_FFT == 0) {
//void r4_pass2_5120_levels_real(struct gwasm_data *__restrict g)
	...
	if likely(!(g->ffttype & 4)) { //xpass2_5120_levels_real_4;
		...
		for(unsigned int loopA = 4; loopA; loopA--) {
			for(unsigned int loopB = 128; loopB; loopB--) {
				r5_fh5cl_ten_reals_five_complex_djbfft(rsi, 64, 4*dist128, rdi, 512*XMM_SCD2, rdx, 512*XMM_HSCD2);
		...
		rsi = daddr; //rsi += -4*4*dist128-4*dist128; /* Restore source pointer */
		...
		rsi = daddr; //rsi += -10*2*dist128;	/* Next source pointer */
		...
		r4_x4cl_four_complex_djbfft_preload;
		for(unsigned int loopA = 20*2-1; loopA; ) {
			do{
		...
		rsi = daddr; //rsi += -20*dist128;	/* Next source pointer */
		...
		for(unsigned int loopA = 4; loopA; loopA--) {
			...
			rdi += 16*XMM_SCD2;	/* Next si	n/cos data */
		...
		r4_x4cl_four_complex_djbfft_preload;
		for(unsigned int loopA = 20*8-1; loopA; ) {
			do{
		...
		if likely(g->ffttype & 2) {
			if likely(!(g->ffttype & 1)) { // xpass2_5120_levels_real_2:;
				...
				for(unsigned int loopA = 20*32-1; loopA; ) {
					do{
						r4_x4cl_four_complex_with_square(rsi, 4*64, 64, 2*64);
					}while(--loopA & 31);
					rsi += -32*4*64+dist128;	/* Next source pointer */
				}
				//rsi += -20*dist128;	/* Next source pointer */
			}else{ // xpass2_5120_levels_real_3:;
		...
		}else{ // xpass2_5120_levels_real_1:;
			...
			goto	xpass2_5120_real_done;
		}
	}else{ // xpass2_5120_levels_real_4:;
		rbp = g->DIST_TO_MULSRCARG;
	...
	for(unsigned int loopA = 20*16-1; loopA; ) {
		do{
	...
	r4_x4cl_four_complex_djbunfft_preload;
	for(unsigned int loopA = 20*4-1; loopA; ) {
		do{
	...
	for(unsigned int loopA = 128; loopA; loopA--) {
		r4_h4cl_eight_reals_four_complex_djbunfft(rsi, 64, dist128, 2*dist128, rdi, 128*5*XMM_SCD2, rdx, 128*XMM_HSCD2);
		rdi += 5*XMM_SCD2;		/* Next sin/cos data */
		rdx += XMM_HSCD2;		/* Next sin/cos data */
	}
	rsi += -128*64+4*dist128;	/* Next source pointer */
	rdi += -128*5*XMM_SCD2;	/* Restore sin/cos ptr */
	rdx += -128*XMM_HSCD2-1024*XMM_HSCD2; /* Restore sin/cos ptr */
	...
	for(unsigned int loopA = 4; loopA; loopA--) {
		for(unsigned int loopB = 128; loopB; loopB--) {
			r4_x4cl_four_complex_djbunfft(rsi, 64, dist128, 2*dist128, rdi, 128*5*XMM_SCD2);
			rdi += 5*XMM_SCD2;		/* Next sin/cos data */
		}
		rsi += -128*64+4*dist128;	/* Next source pointer */
		rdi += -128*5*XMM_SCD2;	/* Restore sin/cos ptr */
	}
	rsi = daddr; //rsi += -5*4*dist128;	/* Next source pointer */
	...
	for(unsigned int loopA = 4; loopA; loopA--) {
		for(unsigned int loopB = 128; loopB; loopB--) {
			r5_h5cl_ten_reals_five_complex_djbunfft(rsi, 64, 4*dist128, rdi, 512*XMM_SCD2, rdx, 512*XMM_HSCD2);
			rdi += XMM_SCD2;		/* Next sin/cos data */
			rdx += XMM_HSCD2;		/* Next sin/cos data */
		}
		rsi += -128*64+dist128;	/* Next source pointer */
...
r4_pass2_5120_levels_complex(g);
}
...
do{
	...
	rsi = daddr;
	rbx = g->DIST_TO_FFTSRCARG;
	if likely(!(g->ffttype & 4)) { //xpass2_5120_levels_complex_4;
		...
		for(unsigned int loopA = 4; loopA; loopA--) {
			for(unsigned int loopB = 128; loopB; loopB--) {
				r5_f5cl_five_complex_djbfft(rsi, 64, 4*dist128, rdi, 512*XMM_SCD2);
				rdi += XMM_SCD2;		/* Next sin/cos data */
			}
			rsi += -128*64+dist128;	/* Next source pointer */
		}
		...
		rsi = daddr; //rsi += -5*4*dist128;	/* Restore source pointer */
		end_timer(5);
		...
		for(unsigned int loopA = 2; loopA; loopA--) {
			for(unsigned int loopB = 128; loopB; loopB--) {
				r5_f5cl_five_complex_djbfft_preload;
				for(unsigned int loopC = 2; loopC; loopC--) {
					r5_f5cl_five_complex_djbfft(rsi, 2*dist128, 4*dist128, rdi, 512*XMM_SCD2);
					xxprefetch(u8ptr(rdi+XMM_SCD2));	/* Prefetch for next pass */
					xxprefetch(u8ptr(rdi+513*XMM_SCD2));	/* Prefetch for next pass */
				...
				for(unsigned int loopC = 5; loopC; loopC--) {
					r4_x2cl_four_complex_djbfft(rsi, 4*dist128, 2*dist128, rbp);
				}

				rsi += -5*4*dist128+64;	/* Next source pointer */
				rdi += -2*256*XMM_SCD2+XMM_SCD2; /* Next sin/cos ptr */
				rbp += 5*XMM_SCD2;		/* Next sin/cos ptr */
			}
			rsi += -128*64+dist128;	/* Next source pointer */
		}
		...
		rsi = daddr; //rsi += -10*2*dist128;	/* Restore source pointer */
		end_timer(6);
		...
		if likely(g->ffttype & 2) {
			if likely(!(g->ffttype & 1)) { // xpass2_5120_levels_complex_2:;
				start_timer(9);
				...
				end_timer(9);
			}else{ // xpass2_5120_levels_complex_3:;
				start_timer(9);
				rbp = g->DIST_TO_MULSRCARG;
				loops_init_prefetch(640, 64, 8, rcx);
		...
		}else{ // xpass2_5120_levels_complex_1:;
			start_timer(9);
			...
			end_timer(9);
			goto	xpass2_5120_levels_complex_done;
		}
	}else{ // xpass2_5120_levels_complex_4:;
		start_timer(9);
		pfing(rcx = (uintptr_t)g->data_prefetch); /* Prefetch pointer */
		xtouch(xptr(rcx));			/* Preload first TLB */
		rbp = g->DIST_TO_MULSRCARG;
	...
	rsi = daddr; //rsi += -20*dist128;	/* Restore source pointer */
	end_timer(10);
	...
	for(unsigned int loopA = 4; loopA; loopA--) {
		for(unsigned int loopB = 128; loopB; loopB--) {
			r5_x5cl_five_complex_djbunfft(rsi, 64, 4*dist128, rdi, 512*XMM_SCD2);
	...
	rdi = (uintptr_t)g->xsincos_complex;	/* Load sin/cos pointer */
	rbp = rdi;
	pfing(rcx += -42*64);		/* Back up prefetch pointer by 42 cache lines */
	...
	for(unsigned int loopA = 128; loopA; loopA--) {
		...
		for(unsigned int loopB = 5; loopB; loopB--) {
			r4_x4cl_four_complex_djbunfft(rsi, 4*dist128, dist128, 2*dist128, rbp, 128*5*XMM_SCD2);
		}
		rsi += -5*4*dist128;	/* Restore source pointer */
		loops_undo(20);

		r5_x5cl_five_complex_djbunfft_preload;
		for(unsigned int loopB = 4; loopB; loopB--) {
			r5_x5cl_five_complex_djbunfft(rsi, dist128, 4*dist128, rdi, 512*XMM_SCD2);
			rdi += 128*XMM_SCD2;	/* Next sin/cos ptr */
		}

		rsi += -4*dist128+64;	/* Next source pointer */
		rdi += -4*128*XMM_SCD2+XMM_SCD2; /* Next sine/cosine pointer */
