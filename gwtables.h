...
struct __attribute__ ((aligned(64))) gwasm_data {
	...
	int 	(*pass1_get_next_block)(struct gwasm_data *g);	/* Callback routine to get next block number for thread to process */
	...
	int 	(*pass2_get_next_block)(struct gwasm_data *g);	/* Callback routine to get next block number for thread to process */
	void	(*thread_work_routine)(struct gwasm_data *__restrict g); /* Assembly routine to call when auxiliary thread wakes up */
	...
	union {
		...
		vec2f64	XMM_TWO;		/* 2.0 */
		vec2f64	XMM_HALF;		/* 0.5 */
		vec2f64	XMM_SQRTHALF;	/* SQRT(0.5) */
		vec2f64	XMM_SUMOUT;		/* Used in normalization macros */

		vec2f64	XMM_MAXERR;		/* Used in normalization macros */
		int32_t	XMM_ABSVAL[4];		/* Used to compute absolute values */
		vec2f64	XMM_BIGVAL;		/* Used to round double to integer */
		vec2f64	XMM_BIGBIGVAL;	/* Used to round to big word multiple */

		vec2f64	XMM_BIGVAL_NEG;
		vec2f64	XMM_MINUS_C;		/* -c stored as double */
		vec2f64	XMM_NORM012_FF;	/* Used in xnorm012 macros (FFTLEN/2) */
		vec2f64	XMM_MULCONST;

		vec2f64	XMM_K_LO;		/* k, bottom big word bits */
		vec2f64	XMM_K_HI;		/* k, top bits */
		vec2f64	XMM_K_TIMES_MULCONST_LO; /* k*mulconst, low big word bits */
		vec2f64	XMM_K_TIMES_MULCONST_HI; /* k*mulconst, top bits */

		vec2f64	XMM_MINUS_C_TIMES_MULCONST;
		vec2f64	XMM_P309;		/* Used in radix-20 macros */
		vec2f64	XMM_P809;
		vec2f64	XMM_P951;

		vec2f64	XMM_P588;
		vec2f64	XMM_P618;		/* Used in old v25 home-grown PFA-5 macros */
		vec2f64	XMM_M809;
		vec2f64	XMM_M262;

		vec2f64	XMM_M382;
		vec2f64	XMM_M162;
		vec2f64	XMM_P866;		/* Used in PFA-6 */
		vec2f64	XMM_P924;		/* Used in old v25 home-grown PFA-5 macros */
		...
		vec2f64	XMM_P180;
		vec2f64	XMM_P975;		/* Used in radix-28 macros */
		vec2f64	XMM_P623;
		vec2f64	XMM_P901;
